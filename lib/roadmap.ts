...
export const ROADMAP: { now: RoadmapItem[]; next: RoadmapItem[]; later: RoadmapItem[] } = {
  ...
  next: [
    ...
    { title: "The published SDK + the Python side", note: "Shipped: **`npm install sanction-sdk`** is live (0.8.0, FSL, zero runtime dependencies — renamed from the planned `@sanction/sdk` scope to publish unscoped like `sanction-mcp`). `SanctionClient` + `SanctionAdminClient`, the escalate→grant loop, and the framework adapters. Python is in-repo: `packages/sanction-python` (same `sanction-sdk` name) ships `SanctionLiteLLMLogger` — LiteLLM usage posted to `/tokens` after the call, httpx-only runtime. Next: PyPI publish, then LangChain/LangGraph + CrewAI over the same client." },
