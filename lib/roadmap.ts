...
export const ROADMAP: { now: RoadmapItem[]; next: RoadmapItem[]; later: RoadmapItem[] } = {
  ...
  next: [
    ...
    { title: "The published SDK + the Python side", note: "@sanction/sdk is publish-ready (0.8.0, FSL, escalate-loop helpers): it builds, typechecks, and passes 55 tests in-repo, and `publish-sdk.yml` is wired to the same npm automation token that ships `sanction-mcp`. The one remaining precondition is the `sanction` npm scope — until it exists, install from `sdk/`. Next: the adapters where Python agents live — a LiteLLM callback and LangChain/LangGraph + CrewAI bindings over the same core, each with a runnable example." },
