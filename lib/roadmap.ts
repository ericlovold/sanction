...
export const ROADMAP: { now: RoadmapItem[]; next: RoadmapItem[]; later: RoadmapItem[] } = {
  ...
  next: [
    ...
    { title: "The published SDK + the Python side", note: "The TypeScript SDK ships as **`sanction-sdk`** (renamed from the planned `@sanction/sdk` scope — unscoped like `sanction-mcp`, so the existing npm automation token publishes it with no org to stand up). 0.8.0, FSL, escalate-loop helpers; builds, typechecks, and passes 55 tests in-repo. One workflow dispatch from live. Next: the adapters where Python agents live — a LiteLLM callback and LangChain/LangGraph + CrewAI bindings over the same core, each with a runnable example." },
