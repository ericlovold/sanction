...
export class SanctionClient {
  ...
  private currentDailyUsd(): number {
   ...
   *
   * Local-first: if Sanction is unreachable (network error, slower than
   * `networkTimeoutMs`, or a 5xx), the SDK decides locally against `localPolicy`.
   * With no local policy it fails closed (deny) unless `failClosed: false`.
   * Genuine auth errors (401/403/400) are NOT masked — they throw.
   */
  ...
  private decideLocally(input: AuthorizeInput): Decision {
    if (this.localPolicy) {
      return localDecision(evaluateLocally(this.localPolicy, input, this.currentDailyUsd()), input)
    }
  ...
  async getStats(walletId: string): Promise<WalletStats> {
    const r = await request<StatsWire>({
      baseUrl: this.baseUrl,
      fetch: this.fetch,
      method: "GET",
      path: "/wallets/stats",
...
/** The snake_case /wallets/stats body. Fields optional — defaulted to 0 on read. */
interface StatsWire {
  today?: { token_cost_usd?: number; tokens_in?: number; tokens_out?: number; spend_usd?: number }
  month?: { token_cost_usd?: number; spend_usd?: number }
