  ...
  const listInput = (name: string, label: string, value: string[], hint: string, danger?: boolean) => (
    <label className="block">
      ...
      <input
        ...
        className={`mt-1 w-full rounded-md border border-border bg-card px-3 py-1.5 font-mono text-sm outline-none focus:border-ring ${danger ? "text-red-300" : "text-foreground"}`}
      />
  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-sm font-medium text-foreground">Policy</CardTitle>
      </CardHeader>
        ...
        <form action={formAction} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3">
            {dollarFields.map((f) => (
              <label key={f.name} className="block">
                ...
                <div className="mt-1 flex items-center rounded-md border border-border bg-card focus-within:border-input">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                className="mt-1 w-full rounded-md border border-border bg-card px-3 py-1.5 font-mono text-sm text-foreground outline-none focus:border-ring [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              />
            ...
            <label className="block">
              ...
              <select
                ...
                className="mt-1 w-full rounded-md border border-border bg-card px-3 py-1.5 text-sm text-foreground outline-none focus:border-ring"
              >
            ...
            <div className="flex items-center justify-between">
              ...
              {editable && rules.length < MAX_CAPABILITY_RULES && (
                <button
                  ...
                  className="text-[11px] font-medium text-signal hover:text-signal"
                >
                  + Add rule
                </button>
              )}
              ...
              {rules.map((r, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-md border border-border bg-card px-3 py-1.5 font-mono text-sm text-foreground outline-none focus:border-ring"
                  />
                  <select
                    value={r.effect}
                    disabled={!editable}
                  ...
                  {editable && (
                    <button
                      ...
                      className="shrink-0 rounded-md border border-border px-2 py-1.5 text-xs text-foreground0 hover:text-red-400"
          ...
          {editable ? (
            <div className="space-y-3">
              <div className="flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  disabled={simulating}
                  className="rounded-md border border-input px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-ring/50 disabled:opacity-50"
                >
            ...
            <div className="flex items-center justify-between gap-3">
              <span className="text-xs text-foreground0">Viewing the demo — log in to edit your own policy.</span>
              <a href="/login" className="rounded-md border border-input px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted">
                Log in
              </a>
            </div>
          )}
