  ...
  return (
    <div className="space-y-3 rounded-md border border-border bg-muted p-4">
      <div className="flex items-baseline justify-between gap-3">
        ...
        <span className="rounded border border-border px-1.5 py-0.5 text-[10px] text-muted-foreground" title="Your real past decisions, replayed under the policy shown — nothing is saved.">
      ...
      {!hasActivity ? (
        ...
        <div className="rounded-md border border-border bg-muted px-3 py-3">
          <p className="text-xs text-muted-foreground">No agent activity in this window yet, so there&rsquo;s nothing to replay.</p>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-md border border-border bg-muted p-3">
              ...
              <p className="mt-2 font-mono text-sm text-foreground">
                ...
                <span className={`ml-2 text-xs ${spendDelta > 0 ? "text-red-400" : spendDelta < 0 ? "text-emerald-400" : "text-muted-foreground"}`}>
                  ...
                  {changes.map((c) => {
                    ...
                    return (
                      <tr key={row.id} className="border-t border-border">
                        <td className="py-1 pr-3 whitespace-nowrap text-muted-foreground">{row.at.slice(0, 10)}</td>
                        <td className="py-1 pr-3 text-muted-foreground">{row.agent ?? "—"}</td>
      ...
      {ignored && ignored.length > 0 && (
        <div className="rounded-md border border-border bg-muted px-3 py-2.5">
          <p className="text-[11px] leading-relaxed text-muted-foreground">
            <span className="font-medium text-muted-foreground">These settings don&rsquo;t affect this result.</span> A spend
          ...
          <p className="mt-1.5 text-[11px] text-muted-foreground">
            Unaffected: <span className="text-muted-foreground">{ignored.map(humanField).join(" · ")}</span>
          </p>
        </div>
      )}
      {truncated && (
