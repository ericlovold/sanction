  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-sm font-medium text-foreground">Policy packs</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4 px-5 pb-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {POLICY_PACKS.map((pack) => (
            <div key={pack.id} className="flex flex-col gap-2 rounded-md border border-border bg-muted/40 p-3">
              <div className="flex items-baseline justify-between gap-2">
                <span className="text-sm font-medium text-foreground">{pack.name}</span>
                <span className="rounded border border-input px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-foreground0">
                  {maturityLabel[pack.maturity]}
                </span>
              </div>
              <p className="text-[11px] leading-relaxed text-foreground0">{pack.tagline}</p>
              ...
              <div className="mt-auto flex gap-2 pt-1">
                <form action={previewFormAction}>
                  ...
                  <button
                    ...
                    className="rounded-md border border-input px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:border-ring/50 disabled:opacity-40"
                  >
                ...
                >
                  ...
                  <button
                    ...
                    className="rounded-md bg-signal px-3 py-1.5 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-40"
                  >
