    ...
    <div className="mx-auto min-h-screen max-w-6xl space-y-6 p-6">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <Link
          ...
          className={`rounded border px-2 py-1 ${scope === "wallet" ? "border-signal/25 bg-signal/10 text-signal" : "border-border text-foreground0 hover:text-foreground"}`}
        >
          Current wallet
        </Link>
        <Link
          href={`/dashboard/agents?scope=subtree${stateFilter !== "all" ? `&state=${stateFilter}` : ""}`}
          className={`rounded border px-2 py-1 ${scope === "subtree" ? "border-signal/25 bg-signal/10 text-signal" : "border-border text-foreground0 hover:text-foreground"}`}
        >
        ...
        {(["all", "inactive", "expiring", "expired"] as const).map((state) => (
          <Link
            ...
            className={`rounded border px-2 py-1 ${stateFilter === state ? "border-input bg-muted text-foreground" : "border-border text-foreground0 hover:text-foreground"}`}
          >
            {state}
          </Link>
        ))}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Card className="border-border bg-card">
          <CardHeader className="px-4 pt-4 pb-1">
            <CardTitle className="text-xs font-normal text-foreground0">Active agents</CardTitle>
          </CardHeader>
        ...
        <Card className="border-border bg-card">
          <CardHeader className="px-4 pt-4 pb-1">
            <CardTitle className="text-xs font-normal text-foreground0">Active grants</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
        ...
        <Card className="border-border bg-card">
          <CardHeader className="px-4 pt-4 pb-1">
            <CardTitle className="text-xs font-normal text-foreground0">Pending approvals</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
        ...
        <Card className="border-border bg-card">
          <CardHeader className="px-4 pt-4 pb-1">
            <CardTitle className="text-xs font-normal text-foreground0">Expiring in 7d</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
        ...
        <Card className="border-border bg-card">
          <CardHeader className="px-4 pt-4 pb-1">
            <CardTitle className="text-xs font-normal text-foreground0">Inactive / expired</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
      ...
      {view.isSession ? (
        ...
        <p className="text-sm text-foreground0">
          <Link href="/login" className="text-signal hover:text-signal">Log in</Link> to create and manage agents.
        </p>
      )}
