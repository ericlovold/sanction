...
function barTone(p: number) {
  ...
  if (p >= 80) return { bar: "bg-ochre", text: "text-ochre" }
  ...
  return (
    <div>
      ...
      {p >= 100 ? (
        ...
        <p className={`mt-1 text-[11px] ${pace.willExhaust ? "text-ochre" : "text-muted-foreground"}`}>
    ...
    <div className="min-h-screen max-w-6xl mx-auto space-y-6 p-6">
      ...
      <Card className="bg-card border-border">
        <CardHeader className="px-5 pt-5 pb-1">
          <CardTitle className="text-sm font-medium text-foreground">Today against budget</CardTitle>
        </CardHeader>
        ...
        {[
          ...
          <Card key={k.label} className="bg-card border-border">
            <CardHeader className="px-4 pt-4 pb-1">
              <CardTitle className="text-xs font-normal text-foreground0">{k.label}</CardTitle>
            </CardHeader>
      ...
      <Card className="bg-card border-border">
        <CardHeader className="px-5 pt-5 pb-2">
          <CardTitle className="text-sm font-medium text-foreground">Token cost · last 14 days</CardTitle>
        </CardHeader>
          ...
          <div className="flex h-32 items-end gap-1.5">
            {s.days.map((d, i) => (
              <div key={i} className="group flex flex-1 flex-col items-center justify-end gap-1.5">
                ...
                <div
                  className="w-full rounded-sm bg-signal/70 group-hover:bg-primary/90 transition-colors"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {/* By model */}
        <Card className="bg-card border-border">
          <CardHeader className="px-4 pt-4 pb-2"><CardTitle className="text-sm font-medium text-foreground">By model · this month</CardTitle></CardHeader>
          <CardContent className="px-4 pb-4">
            {s.byModel.length === 0 && (
              <EmptyState
              ...
              {s.byModel.map((m) => (
                <div key={m.model} className="flex items-center justify-between text-sm">
                  <div className="min-w-0">
                    <p className="truncate font-mono text-xs text-foreground">{m.model}</p>
        ...
        {/* By agent */}
        <Card className="bg-card border-border">
          <CardHeader className="px-4 pt-4 pb-2"><CardTitle className="text-sm font-medium text-foreground">By agent · this month</CardTitle></CardHeader>
          <CardContent className="px-4 pb-4">
                ...
                <div key={a.name} className="flex items-center justify-between text-sm">
                  <div className="min-w-0">
                    <p className="flex items-center gap-1.5 truncate text-foreground">
                      {a.name}
                      {a.override && (
                        <span title={a.override} className="rounded border border-signal/25 bg-signal/10 px-1 py-0.5 text-[9px] font-medium text-signal">
                          custom budget
                        </span>
                      )}
                    </p>
                    <p className="text-[11px] text-muted-foreground">
                      ...
                      {a.escalated > 0 && <span className="text-ochre/70"> · {a.escalated} esc</span>}
                    </p>
                  </div>
        ...
        {/* By task */}
        <Card className="bg-card border-border">
          <CardHeader className="px-4 pt-4 pb-2"><CardTitle className="text-sm font-medium text-foreground">By task · this month</CardTitle></CardHeader>
          <CardContent className="px-4 pb-4">
            {s.byTask.length === 0 && (
              <EmptyState
            ...
            <div className="space-y-2">
              {s.byTask.map((t) => (
                <div key={t.taskLabel ?? "unlabeled"} className="flex items-center justify-between text-sm">
                  <p className="truncate text-foreground">{t.taskLabel ?? "unlabeled"}</p>
        ...
        {/* By category (spend) */}
        <Card className="bg-card border-border">
          <CardHeader className="px-4 pt-4 pb-2"><CardTitle className="text-sm font-medium text-foreground">Approved spend by category · month</CardTitle></CardHeader>
          <CardContent className="px-4 pb-4">
      ...
      <Card className="bg-card border-border">
        <CardContent className="flex items-center justify-between px-5 py-4">
          ...
          <Link href="/dashboard/policy" className="rounded-md border border-input px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted">
            Manage policy →
          </Link>
        </CardContent>
      </Card>
