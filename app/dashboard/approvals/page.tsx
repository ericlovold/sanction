...
const statusClasses: Record<string, string> = {
  ...
  denied: "bg-red-500/15 text-red-700 dark:text-red-400 border-red-500/25",
  ...
  return (
    <div className="min-h-screen max-w-6xl mx-auto space-y-6 p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Card className="bg-card border-border">
          ...
          <CardContent className="px-4 pb-4">
            <p className={`font-mono text-2xl font-semibold ${pending.length > 0 ? "text-amber-600 dark:text-amber-300" : ""}`}>{pending.length}</p>
          </CardContent>
        </Card>
        <Card className="bg-card border-border">
          <CardHeader className="px-4 pt-4 pb-1">
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent className="px-4 pb-4">
            <p className={`font-mono text-2xl font-semibold ${expiringSoon > 0 ? "text-amber-600 dark:text-amber-300" : ""}`}>{expiringSoon}</p>
      ...
      <div className="flex items-center gap-2">
        ...
        {pending.length > 0 && (
          <Badge className="bg-amber-500/15 text-amber-700 dark:text-amber-400 border border-amber-500/25">{pending.length}</Badge>
        )}
      </div>
            ...
            {resolved.map((r) => (
              <div key={r.id} className="flex items-center justify-between text-sm">
                ...
                <div className="ml-3 flex shrink-0 items-center gap-2">
                  {r.grants[0] ? (
                    <span
                      className="rounded border border-emerald-500/25 bg-emerald-500/[0.08] px-1.5 py-0.5 text-[10px] font-medium text-emerald-700 dark:text-emerald-300"
