...
export default function MoralIntentionAnalyst() {
  return (
    <main className={`sanction ${brandFontVars}`} style={{ minHeight: "100vh" }}>
      ...
      {/* Approach — GTM value, no operational framework detail */}
      <section id="approach" className="mia-field" style={{ borderTop: "1px solid var(--line-2)" }}>
        <div style={{ ...wrap, padding: "88px 32px" }}>
          <Reveal style={{ maxWidth: 660, margin: "0 auto 48px", textAlign: "center" }}>
            <h2 style={{ margin: 0, font: "var(--text-h1)", letterSpacing: "var(--tracking-heading)" }}>What it does for you.</h2>
            <p style={{ fontSize: 15.5, color: "var(--text-secondary)", margin: "14px 0 0" }}>
              A structured second set of eyes on the decisions that carry ethical weight.
            </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 20, maxWidth: 980, margin: "0 auto" }}>
            {VALUE.map(({ title, body }, i) => (
              <Reveal key={title} delay={(i % 2) * 80}>
                <div className="mia-lift" style={{ padding: "26px 26px", borderRadius: "var(--radius-card)", background: "var(--surface-card)", border: "1px solid var(--line-1)", borderTop: `3px solid ${i % 2 ? "var(--ochre-6)" : "var(--pine-7)"}`, height: "100%" }}>
                  <div className="sn-mono" style={{ color: i % 2 ? "var(--ochre-7)" : "var(--pine-7)", fontSize: 12, letterSpacing: "0.08em", marginBottom: 10 }}>{`0${i + 1}`}</div>
      ...
      <section style={{ borderTop: "1px solid var(--line-2)" }}>
        <div style={{ ...wrap, padding: "80px 32px", maxWidth: 820, textAlign: "center" }}>
          <Reveal>
            <div className="sn-mono" style={{ color: "var(--ochre-6)", letterSpacing: "0.1em", marginBottom: 14 }}>THE FOUNDATION</div>
      ...
      {/* AI thesis — high-level positioning only */}
      <section style={{ borderTop: "1px solid var(--line-2)", background: "var(--pine-9)", color: "#f7f6f0" }}>
        <div style={{ ...wrap, padding: "84px 32px", maxWidth: 820, textAlign: "center" }}>
          <Reveal>
            <div className="sn-mono" style={{ color: "var(--ochre-6)", letterSpacing: "0.1em", marginBottom: 12 }}>WHY IT MATTERS NOW</div>
            ...
            <p style={{ fontSize: 16, lineHeight: 1.7, color: "rgba(247,246,240,.8)", margin: "18px auto 0", maxWidth: "60ch" }}>
              An automated system can act thousands of times before a human notices, reach millions at once, and feel no
              discomfort that would make it stop. AI cannot hold moral intention itself — people must. MIA is the layer
              that keeps that intention visible, so human judgment stays in the loop where it belongs.
            </p>
          </Reveal>
        </div>
      </section>
