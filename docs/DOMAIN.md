...
| **Execution Token / Mandate** | Shipped | `ExecutionToken` | Short-lived (15-min TTL) scoped JWT (`jti` = id). Bounds which credentials an execution may inject and its spend authority. Product-facing name for the same object when presented to a child agent or counterparty: **Mandate**. Counterparties verify liveness at `POST /v1/mandate/verify` (WALLET-1) without an API key. |
| **Wallet Card** | Shipped | *(discovery document)* | Platform discovery at `GET /.well-known/wallet-card.json` — carry, present, verify, evidence, and the honesty contract. Not a Prisma model. Per-agent cards are Next. |
