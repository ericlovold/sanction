...
| **Version literals live in more places than you expect.** | `package.json`, `packages/sanction-mcp/package.json`, `sdk/package.json`, `lib/walletCard.ts`, `public/.well-known/mcp.json`, `packages/sanction-mcp/server.json`. Bump with `/cut-release`, not by hand. |
