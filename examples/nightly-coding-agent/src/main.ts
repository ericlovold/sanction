...
async function main() {
  ...
  // Apply the blueprint policy in ONE call (partial update; cents in/out).
  const admin = new SanctionAdminClient(managementKey, opts)
  ...
  for (const item of backlog) {
    line(`\n• ${item.task}  (${usd(item.amountUsd * 100)}, category=${item.category})`)
    ...
    await branchOnDecision(decision, {
      onApproved: async () => {
        ...
        line(`  → Logged token usage (${usd(item.amountUsd * 100)}).`)
      },
      onEscalated: () => {
        escalatedCount++
